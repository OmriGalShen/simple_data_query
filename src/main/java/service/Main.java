...
public class Main {
    ...
    private static void printHelp(){
        System.out.println(
                    "| EQUAL(property,value)  Filters only values which have matching property value.| EQUAL(id,\"s1\")| \n" +
                    ...
                    "| AND(a,b)  Filters only values for which both a and b are true.| AND(EQUAL(id,\"s1\"),EQUAL(views,100)) | \n" +
                    "| OR(a,b)  Filters only values for which either a or b is true (or both).| OR(EQUAL(id,\"s1\"),EQUAL(id,\"s2\")) | \n" +
                    "| NOT(a)  Filters only values for which a is false.| NOT(EQUAL(id,\"s1\")) | \n" +
                    ...
                    "| UPDATE(property,value,a)   Update property where a is true| UPDATE(views,500,EQUAL(id,\"s1\")) | \n" +
                    "| DELETE(a)   Delete values where a is true| DELETE(EQUAL(id,\"s1\")) | \n" +
